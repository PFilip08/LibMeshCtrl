...
const https_proxy_agent__default = /*#__PURE__*/_interopDefaultCompat(https_proxy_agent);
const crypto__default = /*#__PURE__*/_interopDefaultCompat(crypto);
const fs__default = /*#__PURE__*/_interopDefaultCompat(fs);
const ws__default = /*#__PURE__*/_interopDefaultCompat(ws);
const urllib__default = /*#__PURE__*/_interopDefaultCompat(urllib);
...
class Session {
    ...
    constructor(url, {user=null, domain=null, password=null, loginkey=null, proxy=null, token=null, ignoreSSL=false}) {
        ...
        if (loginkey) {
            try {
                loginkey = fs__default.readFileSync(loginkey);
            } catch (err) {
                if (err.code == "ENOENT") ;
                else {
                    throw err
        ...
        this._eventer = new EventEmitter__default();
    ...
    _initialize() {
        ...
        if (this._proxy != null) {
            options.agent = new https_proxy_agent__default(urllib__default.parse(this._proxy));
        }
        if (this._password) {
            let token = this._token || "";
            options.headers = { 'x-meshauth': Buffer.from('' + this._user).toString('base64') + ',' + Buffer.from('' + this._password).toString('base64') + token };
        }
        this._sock = new ws__default(this.url, options);
            ...
            let l = this.listen_to_events((data)=>{
                if (match_nodeid(data.nodeid, nodeids)) {
                    if (data.value === "Run commands completed.") {
                        ...
                        if (___default.every(Object.entries(result).map(([key, o])=>o.complete))) {
    ...
    async upload_file(nodeid, filepath, target, unique_file_tunnel=false) {
        let f = fs__default.createReadStream(filepath);
    ...
    async download(nodeid, source, target=null, unique_file_tunnel=false) {
        if (target===null) {
            target = new stream__default.PassThrough();
        }
        let files = await this.file_explorer(nodeid, unique_file_tunnel);
    ...
    async download_file(nodeid, source, filepath, unique_file_tunnel=false) {
        let f = fs__default.createWriteStream(filepath);
    ...
    _getRandomAmtPassword() { var p; do { p = Buffer.from(crypto__default.randomBytes(9), 'binary').toString('base64').split('/').join('@'); } while (this._checkAmtPassword(p) == false); return p; }
    _getRandomHex(count) { return Buffer.from(crypto__default.randomBytes(count), 'binary').toString('hex'); }
        ...
        this._session._send_command_no_response_id({ "action":"authcookie" }).then((data)=>{
            ...
            this._session._send_command({ action: 'msg', nodeid: this.node_id, type: 'tunnel', usage: 1, value: '*/meshrelay.ashx?p=' + this._protocol + '&nodeid=' + this.node_id + '&id=' + this._tunnel_id + '&rauth=' + data.rcookie }, "initialize_shell").then((data2)=>{
                ...
                if (this._session._proxy != null) {
                    options.agent = new https_proxy_agent__default(urllib__default.parse(this._session._proxy));
                }
...
class _SizeChunker extends stream__default.Transform {
...
exports.CONSENTFLAGS = CONSENTFLAGS;
exports.ERRORS = ERRORS;
exports.ICON = ICON;
exports.MESHFEATURES = MESHFEATURES;
exports.MESHRIGHTS = MESHRIGHTS;
exports.PROTOCOL = PROTOCOL;
exports.SHARINGTYPE = SHARINGTYPE;
exports.SHARINGTYPENUM = SHARINGTYPENUM;
